...
private[stat] object KSTest {
  ...
  /**
   *  Implements a 2 sample, 2 sided Kolmogorov Smirnov test
   * @param data1 `RDD[Double]` first sample of data
   * @param data2 `RDD[Double]` second sample of data
   * @return A KSTestResult with the test statistic, p-value (calculated with the 2 sample sizes),
   *         and appropriate null hypothesis
   */
   ...
   * the 2 sample Kolmogorov Smirnov test implementation
  ...
  private def searchTwoSampleCandidates(
      ...
      n2: Double)
    : Iterator[(Double, Double, Double)] = {
    // fold accumulator: local minimum, local maximum, index for sample1/sample2
   ...
   * the 2-sample, 2-sided Kolmogorov Smirnov test
   ...
   *         of sample 2)
   * @return The 2-sample, 2-sided Kolmogorov Smirnov statistic
   */
